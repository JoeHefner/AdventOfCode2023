...
const numberMatch = [{key:'zero', value: 0}, {key:'one', value: 1}, {key:'two',value:2}, {key:'three',value:3}, {key:'four',value:4}, {key:'five',value:5}
, {key:'six',value:6}, {key:'seven',value:7}, {key:'eight',value:8}, {key:'nine',value:9}]
const regexPattern = new RegExp(/(?=(\d|one|two|three|four|five|six|seven|eight|nine))/g);
let rawCoordinates = [];
let coordinateOutput = 0;
console.log(regexPattern)
function extractDataInput(line){
    
    let data = [...line.matchAll(regexPattern)];
    console.log(data);
    let dataArray = [];
    let newCoordinates = [];
    data.forEach((entry) => {
        if(isNaN(entry[1])){
            dataArray.push(convertAlphaToNumeric(entry[1]))
        }
        else{
    ...
    rawCoordinates.push(dataArray);
}

function convertAlphaToNumeric(alpha){
    let numericValue = ''

    numberMatch.forEach((entry) =>{
        if(entry.key === alpha){
...
function processCoordinates(entry){
    let coordinate = 0;
    if (entry.length === 1){
        coordinate = +entry[0] * 11;
    }
    else if (entry.length > 1)
    { 
        coordinate = +entry[0] * 10 + +entry[(entry.length - 1)]
    }
    coordinateOutput += coordinate;
}

dataInput.forEach(line => {
    extractDataInput(line);
})

rawCoordinates.forEach(entry => {
